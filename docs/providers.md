...
GamesMcp exposes one MCP server to AI clients and can route selected calls to external game knowledge providers. The first provider is Istaroth for Genshin Impact.
...
Provider code lives in `packages/providers`. Core code only depends on `GameKnowledgeProvider`, `GameProviderRegistry`, and normalized GamesMcp response types. Istaroth-specific MCP output parsing is isolated under `packages/providers/src/istaroth`.
...
If `GAMESMCP_ISTAROTH_ENABLED=false`, GamesMcp starts normally and the provider tools return a standard provider error. Existing local tools are unaffected.
...
The benchmark writes `artifacts/provider-baseline.json` by default and records cold/warm latency, P50/P95/P99, response bytes, success rate, and concurrency 1/4/16.
