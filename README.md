...
外部游戏知识 Provider 网关见 [`docs/providers.md`](docs/providers.md)。支持通过 MCP Streamable HTTP 接入 Istaroth，并保留星铁 `starrail-local` 作为迁移 baseline；目标路径是 TurnBasedGameData → StarRail corpus → Istaroth checkpoint → GamesMcp gateway。统一暴露 `search_game_knowledge`、`get_game_document`、`get_game_document_hierarchy` 和 `get_game_provider_status`，旧工具保持原本本地 PostgreSQL 行为。
