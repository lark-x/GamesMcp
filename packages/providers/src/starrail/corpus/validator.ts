...
export async function validateStarRailIstarothCorpus(input: {
  ...
  for (const entry of manifest) {
    ...
    if (!entry.title?.trim()) errors.push(`empty title: ${entry.relative_path}`);
    if (!Number.isInteger(entry.id) || entry.id < 0)
      errors.push(`invalid id: ${entry.category}:${entry.id}`);
    const idKey = `${entry.category}:${entry.id}`;
    if (seenIds.has(idKey)) errors.push(`duplicate category/id: ${idKey}`);
    seenIds.add(idKey);
    ...
    if (/[A-Za-z0-9_./\\-]+\.(?:png|jpg|jpeg|webp|prefab|asset|wav|ogg|mp3)/iu.test(content))
      warnings.push(`possible asset/path text: ${entry.relative_path}`);
  }
  for (const category of input.requiredCategories ?? REQUIRED_CATEGORIES) {
    if (!categories[category]) errors.push(`required category missing: ${category}`);
  }
