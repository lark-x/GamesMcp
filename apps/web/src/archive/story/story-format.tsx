...
export function formatStoryString(rawText: string, prefs?: ProtagonistPreferences): string {
  ...
  // 4. Isolated {M#...} and {F#...}
  text = text.replace(/\{M#([^}]*)\}/gi, (_m, mVal) => (prefs.gender === "male" ? mVal : ""));
  text = text.replace(/\{F#([^}]*)\}/gi, (_m, fVal) => (prefs.gender === "female" ? fVal : ""));
  ...
  text = text.replace(
    /\{LAYOUT_MOBILE#([^}]*)\}\{LAYOUT_PC#([^}]*)\}\{LAYOUT_PS#([^}]*)\}/gi,
    "$2",
  );
  ...
  text = text.replace(
    new RegExp(
      `(?<=[${PARTICLES_BEFORE}])\\{RUBY#\\[([SD])\\]([^}]+)\\}([\\u4e00-\\u9fa5]{1,2})`,
      "g",
    ),
    (_m, _mode, rt, b) => `<ruby>${b}<rt>${rt}</rt></ruby>`,
  );
 ...
 */
export function formatStoryText(rawText: string, prefs?: ProtagonistPreferences): React.ReactNode {
