...
export function AdminRoutes({ initialRoute }: { initialRoute: string }) {
  ...
  const refreshCandidates = useCallback(async (selectedGameId?: string) => {
    const summaries = (await api.candidates(selectedGameId)).candidates ?? [];
    const detailResults = await Promise.allSettled(
      summaries.map((candidate) => api.candidate(candidate.id)),
    );
    const details = detailResults.map((result, index) =>
      result.status === "fulfilled" ? result.value.candidate : summaries[index]!,
    );
    setCandidates(details);
    const resultPairs = await Promise.all(
      details.map(async (candidate) => {
        const [candidateReadiness, candidateChecks] = await Promise.allSettled([
          api.candidateReadiness(candidate.id),
          api.candidateChecks(candidate.id),
        ]);
        return {
          id: candidate.id,
          readiness:
            candidateReadiness.status === "fulfilled" ? candidateReadiness.value : undefined,
          checks: candidateChecks.status === "fulfilled" ? candidateChecks.value.checks : [],
        };
      }),
    );
    setReadiness(
      Object.fromEntries(
        resultPairs.flatMap((item) => (item.readiness ? [[item.id, item.readiness]] : [])),
      ),
    );
    setChecks(Object.fromEntries(resultPairs.map((item) => [item.id, item.checks])));
    return details;
  }, []);
  ...
  const refreshIssues = useCallback(async () => {
    ...
    setIssues(active);
    const evidenceResults = await Promise.allSettled(active.map((issue) => api.evidence(issue.id)));
    setEvidence(
      Object.fromEntries(
        active.map((issue, index) => [
          issue.id,
          evidenceResults[index]?.status === "fulfilled"
            ? evidenceResults[index].value.evidence
            : [],
        ]),
      ),
    );
  ...
  return (
    <main className="app-shell admin-shell">
      ...
      <nav className="admin-nav" aria-label="资料管理页面">
        {(
          [
            ["intake", "1", "导入数据"],
            ["preview", "2", "预发布与发布"],
            ["issues", "3", `问题审核${issues.length ? ` (${issues.length})` : ""}`],
            ["history", "4", "正式版本历史"],
          ] as const
        ).map(([id, number, label]) => (
      ...
      {page === "preview" && (
        <section className="workflow-banner" aria-label="预发布与发布流程">
          <strong>本页操作流程</strong>
          <ol>
            <li>选择预发布分支</li>
            <li>打开 Build 浏览内容</li>
            <li>处理所有阻塞问题</li>
            <li>门禁通过后发布正式 Revision</li>
          </ol>
        </section>
      )}
      {page === "issues" && (
        <section className="workflow-banner" aria-label="问题审核流程">
          <strong>本页操作流程</strong>
          <ol>
            <li>查看问题记录</li>
            <li>上传游戏内截图</li>
            <li>选择处理动作</li>
            <li>生成 Patch 与新 Build</li>
          </ol>
        </section>
      )}
        ...
        <section className="admin-page" aria-labelledby="intake-heading">
          ...
          <form
            ...
            onSubmit={async (event) => {
              ...
              setMessage("正在创建导入任务…");
              try {
                const created = await api.createImport({ gameId, sourceId, path: sourcePath });
          ...
          >
            ...
            <label className="admin-form-wide">
              ...
              <input
                ...
                placeholder="例如 F:\\Project\\GamesMcp\\data\\imports\\normalized\\genshin-db\\records.json"
              />
            </label>
            ...
            <button type="submit" disabled={!gameId || !sourceId || !sourcePath.trim()}>
              创建导入任务
            </button>
          </form>
            ...
            {imports.length ? (
              imports.slice(0, 12).map((item) => {
                const candidate = candidates.find((entry) =>
                  entry.importBatchIds?.includes(item.id),
                );
                return (
                  <article className="admin-card" key={item.id}>
                    <div className="admin-card-title">
                      <strong>{sourceById.get(item.sourceId)?.name ?? "已登记来源"}</strong>
        ...
        <section className="admin-page" aria-labelledby="preview-heading">
          <div className="admin-page-heading">
            <div>
              ...
              <p>预发布可以自由浏览；只有当前 Build 的所有门禁通过后，才允许生成正式 Revision。</p>
            </div>
            <button
              className="secondary-button"
          ...
          <div className="candidate-list">
            {candidates.length ? (
                ...
                return (
                  <article className="candidate-card" key={candidate.id}>
                    <header>
                      <div>
                        ...
                        <p>
                          ...
                          {currentBuild?.buildNumber ?? "—"} · {currentBuild?.recordCount ?? 0} 条
                        </p>
                      </div>
                      <span
                    ...
                    <div className="release-gates">
                      ...
                      <div className={currentBuild?.contentChecksum ? "gate-pass" : "gate-block"}>
                        <strong>内容校验和</strong>
                        <span>
                          {currentBuild?.contentChecksum
                    ...
                    <div className="candidate-actions">
                      ...
                      <button className="secondary-button" onClick={() => go("issues")}>
                        处理问题
                      </button>
                    </div>
              ...
              <div className="empty-state">
                ...
                <button onClick={() => go("intake")}>前往导入</button>
              </div>
            )}
          </div>
        </section>
      ...
      {page === "issues" && (
        <section className="admin-page" aria-labelledby="issues-heading">
          <div className="admin-page-heading">
            ...
            <button
              className="secondary-button"
              onClick={() => refreshIssues().then(() => setMessage("问题列表已刷新"))}
            >
              刷新问题
            </button>
          </div>
          <div className="issue-list">
            {issues.length ? (
              issues.map((issue) => {
                ...
                const uploaded = evidenceUploaded[issue.id] || Boolean(evidence[issue.id]?.length);
                ...
                return (
                  <article className="issue-card" key={issue.id}>
                    ...
                    <div className="review-form">
                      ...
                      <label className="admin-form-wide evidence-picker">
                        ...
                        />
                        {evidenceFiles[issue.id] && <span>{evidenceFiles[issue.id]?.name}</span>}
                      </label>
                    </div>
              ...
              revisions.map((revision) => {
                ...
                return (
                  <article className="revision-card" key={revision.id}>
                    <header>
                      <div>
                        <span className="eyebrow">REVISION {revision.revisionNumber ?? "—"}</span>
                        ...
                        <button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              setMessage(
                                `已切换到正式版本 #${revision.revisionNumber ?? revision.id}`,
                              );
