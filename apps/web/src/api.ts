...
async function request<T>(path: string, init?: RequestInit): Promise<T> { const token = localStorage.getItem("gip.adminToken"); const r = await fetch(path, { ...init, headers: { "content-type": "application/json", ...(token ? { authorization: `Bearer ${token}` } : {}), ...(init?.headers ?? {}) } }); if (!r.ok) throw new Error(`${r.status} ${r.statusText}`); return r.json() as Promise<T>; }
export const api = {
  games: () => request<{ games: unknown[] }>("/api/games"),
  sources: (gameId?: string) => request<{ sources: unknown[] }>(gameId ? `/api/games/${gameId}/sources` : "/api/admin/sources"),
  imports: () => request<{ imports: unknown[] }>("/api/admin/imports"),
  createImport: (input: { gameId: string; sourceId: string; path: string }) => request<{ id: string; batchId?: string }>("/api/admin/imports", { method: "POST", body: JSON.stringify(input) }),
  ...
  resolve: (id: string, action: string, note: string) => request(`/api/admin/review-issues/${id}/resolve`, { method: "POST", body: JSON.stringify({ action, note }) }),
  revisions: () => request<{ revisions: Revision[] }>("/api/admin/revisions"),
  rollback: (id: string, reason: string) => request(`/api/admin/revisions/${id}/rollback`, { method: "POST", body: JSON.stringify({ reason }) }),
};
