...
async function profileQuest() {
  ...
  const rev = await pool.query(
    "SELECT id, revision_number FROM knowledge.dataset_revisions WHERE is_current = true",
  );
  ...
  console.log(
    "subquests took:",
    (performance.now() - t).toFixed(2) + "ms, rows:",
    subquests.rowCount,
  );
  ...
  console.log(
    "dialogue count took:",
    (performance.now() - t).toFixed(2) + "ms, count:",
  ...
  const participants = await pool.query(
    "SELECT * FROM knowledge.entities " + "WHERE game_id = $1 AND source_key = ANY($2)",
